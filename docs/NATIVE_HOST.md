...
Handles are process-local monotonically allocated integers, never raw Engine pointers or reusable authority tokens. At most128handles can be open. Operations serialize through a process-wide mutex; a handle retains its initial authority until close. The Swift wrapper is deliberately non-Sendable and confined to its owning executor. A caught panic fails the handle registry closed and requires host restart; invalid foreign pointers, allocation exhaustion and process termination are not recoverable Rust errors.

Paths are capped at4096bytes, host JSON at128KiB, principal/graph identifiers at512bytes, writable IDs at128, and input programs at16MiB/16commands. Responses use the engine's exported cumulative precommit materialization limit (currently32MiB) plus4096bytes for the ABI envelope and array separators. The ABI derives its limit from that engine constant. Review caught an initial smaller response cap that could hide a successfully committed operation behind a serialization error; regression tests now prove successful replies above16MiB are returned and engine-budget rejection above32MiB rolls back earlier writes. These are serialized-byte/object bounds, not sandboxed CPU or whole-process RSS quotas.
