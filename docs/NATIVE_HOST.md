...
Local macOS acceptance at protocol 0.13 is recorded in [the macOS report](measurements/2026-09-19-native-swift-macos.json). The same four-stage scenario passed on a dedicated iPhone 17e simulator running iOS 26.5; [the simulator report](measurements/2026-09-19-native-swift-ios.json) records the exact source commit and toolchain. Each stage launches a new process. A first cold boot exceeded the original timeout, and a later install hit a transient CoreSimulator placeholder error. The harness now allows a bounded cold boot and retries that specific install-error domain at most twice before failing. The dedicated simulator is removed after the run. No physical-device, energy, networking-disabled or full peer-sync acceptance follows from this local persistence test.
