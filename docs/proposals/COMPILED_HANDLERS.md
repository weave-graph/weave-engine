...
Implemented candidate protocol **0.18**, SQLite marker **16**, following root approval of the native interface and language `REACTOR_INTERFACE.md`. Canonical checkpoint `4e81a16`, deterministic nested RuleSet source identities `efc4b69`, and native preparation checkpoint `27aa723` are local review boundaries. This document describes the implemented profile and its explicit remaining scope; publication is separate.
...
`GraphData` has no source manifest field. Persist a generated graph-host literal attachment with explicit format `weave-handler-attribution/1`, template digest and canonical source revisions. Give it a collision-checked generated ID, installed readers, the output's explicit context where present, and all three influence kinds. It counts toward byte/reference limits. The attribution and prepared body also retain bounded computed coverage and diagnostics (at most256 diagnostics and64KiB). These carry the same principal and exact-input restrictions as the output. Later ordinary Query coverage describes current visibility of the stored snapshot; it does not reconstruct or override coverage from arbitrary attribution metadata. Full persisted computation-envelope semantics remain open. This is ordinary descriptive metadata: it does not authenticate arbitrary same-key payloads and does not manufacture `QueryResult.source_revisions` when later queried. The trusted installed template and preparation remain the execution binding.
...
Private tables: `compiled_handlers(adapter PRIMARY KEY REFERENCES dispatch_adapters, principal, registration, binding_digest)` and `handler_preparations(adapter, event_id, principal, preparation_id UNIQUE, body, body_digest, PRIMARY KEY(adapter,event_id))`. Preparation IDs are opaque durable random occurrence IDs, not hashes of hidden inputs. The registration body contains the complete template, output binding and exact manifest. Registration and preparation digests have distinct versioned domains. The internal canonical preparation body contains a format tag, adapter/event/source identity, template and config binding, output mapping/CAS, canonical manifests, exact input closure, prepared-at time and exact Program. Prepared-at time is descriptive only; it never supplies current authority.
...
The initial native checkpoint passes10 implementation tests plus the3 independent root cases. Identity output retains exact original node, edge/assertion and attachment references; original OR groups remain alternatives. Additional current-governance expiry coverage denies both fresh completion and historical replay. The dispatcher9, governance20 and storage7 affected suites and strict native all-target/all-feature lint passed before final workspace verification. The subsequent combined workspace all-feature run passed405 test/doctest checks, with no failures. These totals apply to this candidate only.

The actual language fixture exercises linked Metadata→Reason, committed and accepted occurrences, empty/unknown outputs, attribution and reader-stripped proof persistence, renewed leases, stale CAS, raw bypass rejection, and actual before/after preparation/completion process deaths. The companion source script reports its process counts; the fixture is trusted test administration, not a remote authority API.

`check_snapshot_migration.py` now accepts `--old-protocol`, `--old-marker` and `--new-marker`. Reports [0.16→16](../measurements/2026-09-20-handler-migration-016.json) and [0.17→16](../measurements/2026-09-20-handler-migration-017.json) use preserved actually executed binaries, with genuine matching historical sealed templates and signed responses. Each verifies precommit rollback, postcommit death/restart, unchanged prior rows and exact cached response replay, new-table atomicity, and old-binary refusal. No baseline rebuild or historical byte rewrite was needed.

Final native commands (one existing cache, `CARGO_BUILD_JOBS=2`, `nice -n 10`):

```sh
cargo test --workspace --all-features --locked
cargo clippy --workspace --all-targets --all-features --locked -- -D warnings
cargo build --locked -p weave-engine --features recovery-testing --bin weave-engine --example source_view_fixture --example compiled_handler_fixture --example storage_probe --example three_peer_trace
cargo fmt --all -- --check
```

Source151 tests, strict lint/fmt and8,723 identical native/WASM artifact bytes were reported by the compiler owner at this candidate. Independent parent execution repeated the actual handler process trace and both populated migration profiles. Hosted CI belongs to the later paired publication; no hosted0.18 result is claimed here.
