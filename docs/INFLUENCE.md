...
Independent compatibility review is complete. Root reran all 284 workspace tests and strict lint, plus the unchanged-source nested selector suite and the 0.14/0.15 host/projection matrix. The historical metadata CLI check now verifies a distinct wrapper ID, unchanged entity/space identity, the exact original NodeRef dependency and empty immutable-source origins; all ten checks pass. Direct target queries still retain their original local IDs. Projection continues to require current result local IDs and fails explicitly with atomic rollback for a missing original ID.

The paired source compiler is `5c3e9095528deee38a8176424621ebe8212085e5`: root verified 102 tests, exact 24-file vendor integrity, strict lint and actual module/schema/live-reference runtime workflows. Hosted integration pins this source revision and runs the nested metadata regression. These results verify the bounded 0.15 checkpoint; the full white-paper acceptance gates remain open.
