...
Local verification before independent freeze: 278 all-feature workspace tests passed, followed by the additional passing node-only cluster-edge regression; strict all-target/all-feature lint passed. The eight existing native/WASM fixtures executed identically (21,161 bytes). Actual old 0.14 binary refusal of a schema-11 store, all three migration process-death checks and valid fuzz seed execution passed. These are local checks, not hosted publication evidence.
