...
Status: adopted and implemented locally in contract 0.4; signed transport remains open.
...
`CommitBatch` allocates `logical:{batch}:{graph}` revisions, binds content digests through an immutable sorted snapshot manifest, rejects equivocation and validates required cyclic dependencies atomically. The independently authored `scripts/root_metadata.py` constructs a real three-graph cycle and verifies replay, privacy and temporal path semantics. Legacy content-hash revisions retain their original encoding. Capsule 0.1 explicitly rejects logical-manifest roots rather than losing the manifest; signer identity and manifest transport remain separate work.
