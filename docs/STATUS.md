...
The portable `weave-cluster` foundation provides lazy topology aggregation, stable membership IDs plus snapshot-bound record revisions, semantic expansion and explicit evidence/budget boundaries. Seven tests cover evidence preservation, deterministic layout, separate overlapping perspectives and malformed context rejection. This is a trusted-host core; authorized native graph operations, incremental updates, lineage, hysteresis and quality/recall measurements remain open. See [clustering boundaries](CLUSTERING.md).
