...
Add clock-driven entry points for governance proposal, approval, acceptance, inspection, and governance delivery. Retain existing explicit-`now` Rust entry points as deprecated compatibility wrappers, with the old argument explicitly ignored for authority. Their implementation delegates to the same captured-clock operation. This preserves call shape but deliberately changes callers that used a supplied timestamp to simulate time; those callers must install a test clock. Do not keep a second backdatable path that can create graph-exposed decisions.
