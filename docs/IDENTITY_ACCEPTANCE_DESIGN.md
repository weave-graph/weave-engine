...
- CAS conflicts and replayed decision bodies preserve a single accepted occurrence. `scripts/root_identity.py` terminates actual processes before commit and after commit before response, then verifies rollback or exact receipt replay from fresh processes. Graph, event, registry, membership, schema and receipt writes share one SQLite savepoint.
...
The reserved `weave:identity:` graph namespace rejects raw commits, batch commits, capsule receipt and branch acceptance. Only the private validated acceptance route can write it. Current policy guards apply to original and pinned queries, metadata, assertion and node proofs, exact resolution and capsule export. Historical pins do not bypass revocation. Stored views, prior-result change guards, adapter event delivery and signed cached-read retries also revalidate current policy; AllowStale never grants stale authority.

All membership queries, metadata expansion and resolver values return the same generic `Partial` coverage diagnostic because they enumerate only authorized and available records, never a global directory. Primitive reader checks happen before source-proof traversal; hidden members cannot change visible payload, layout or this coverage marker. Mapping revisions still expose that a decision version changed; timing, resource use and full activity hiding are not claimed.
...
Root recovery evidence: six actual-process checks pass. Exit83 after acceptance SQL but before RELEASE leaves the prior two source revisions/events and candidate intact with no decision, membership, registry, receipt or membership schema. Exit84 after commit simulates a lost response; fresh retries return the exact durable receipt (with duplicate=true) and retain one decision event. Changed-body nonce reuse fails. Fresh resolution preserves independent IDs, and policy revocation rejects pinned resolution and receipt retry. This proves the local SQLite boundary, not network exactly-once delivery or remote governance synchronization. The pre-commit callback is compiled only with recovery-testing and is absent from ordinary builds.
