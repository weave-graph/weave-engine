...
with tempfile.TemporaryDirectory(prefix='weave-selection-migration-') as tmp:
    ...
    with sqlite3.connect(db) as c:
        ...
        assert c.execute("SELECT count(*) FROM sqlite_master WHERE name='view_selection'").fetchone()[0] == 0
    ...
    with sqlite3.connect(db) as c:
        ...
        assert c.execute("SELECT count(*) FROM sqlite_master WHERE name='view_selection'").fetchone()[0] == 1
