...
Implemented: immutable SQLite graph snapshots with optimistic concurrency, distinct entity/space/manifestation IDs, first-class edges, graph-valued node and edge metadata, pinned temporal queries and exact identity-space temporal path joins, transitive provenance visibility, atomic graph commits and durable events, and a reference audit adapter with retries, dead letters and deduplicated logical effects.
...
See [current evidence and limitations](docs/STATUS.md), [implementation plan](docs/IMPLEMENTATION_PLAN.md), [workflow DAG](docs/WORKFLOW.md), [contract](docs/contract/v0.2/README.md) and [source provenance](docs/SOURCES.md). General joins, synchronization/capsules, signed capabilities/governance, real mobile/browser persistence, geometry and automatic clustering remain open. The original white papers remain to be reconciled. MIT licensed.
